...
class Api_sub_fasilitas extends \Restserver\Libraries\REST_Controller
{
...
// constructor
    public function __construct($config = 'rest')
    {
        ...
        $this->load->model('Fasilitas_sub_jenis_model');
    ...
    public function index_get()
    {
        ...
        $data = $this->Fasilitas_sub_jenis_model->get_All();
        $this->response(
            ['msg_main' => [
                'status' => true,
    ...
    {
        $id = $this->post('id');
        $data = $this->Fasilitas_sub_jenis_model->get_byId($id);
        $this->response(
            ['msg_main' => [
                'status' => true,
                'msg' => "get data success",
    ...
    {
        $data = $this->Fasilitas_sub_jenis_model->ket_jenis_get();
        $this->response(
            ['msg_main' => [
                'status' => true,
                'msg' => "get data success!",
    ...
    {
        $id = $this->Fasilitas_sub_jenis_model->get_id();
        // var_dump($id);
        $data = array(
            'id_sub' => $id[0]['id'],
        ...
        if ($is_valid) {
            $query = $this->Fasilitas_sub_jenis_model->post_All($data);
            if ($query === true) {
                $this->response(
                    ['msg_main' => [
                        'status' => true,
    ...
    public function index_put()
    {
        ...
        if ($is_valid) {
            $query = $this->Fasilitas_sub_jenis_model->put_byId($id, $data);
            if ($query === true) {
                $this->response(
                    ['msg_main' => [
                        'status' => true,
    ...
    {
        $id = $this->delete('id');
        $query = $this->Fasilitas_sub_jenis_model->delete_byId($id);
        if ($query === true) {
            $this->response(
                ['msg_main' => [
                    'status' => true,
